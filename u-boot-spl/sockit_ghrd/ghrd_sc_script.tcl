...
# phy_1 is an secure jtag master in design, and phy_2 is fpga to hps sdram jtag master
...
for {set i 0} {$i<[llength $all_sp]} {incr i} {
  ...
  if {[ regexp {/5CS.+/phy_0/} "$sp($i)" ]} {
    ...
    set mj_f2s "$sp($i)"
    puts "f2s jtag master    : $mj_f2s"
    open_service master $mj_f2s
    }  
...
proc regwr_f2s {addr char} {
  global mj_f2s
  master_write_32 $mj_f2s $addr $char
  puts "written to address: 0x[ format %x [ expr $addr ] ]"
}

proc regrd_f2s {addr} {
  global mj_f2s
  set rdata [master_read_32 $mj_f2s $addr 1]
  puts "readdata: $rdata, from addr: $addr"
  return $rdata
}


# procedure to write random data to incremental address of hps DDR memory (via the f2s bridge) and read back for comparison
proc mem_incr_test_f2s {startaddr {count 1}} {
  global mj_f2s
  global hps_mem_base
  set mm_count 0
  set mt_count 0
  for {set oc 0} {$oc<$count} {incr oc} {
    set dataarray($oc) [ expr {int(rand()*4294967296)} ]
    set addrarray($oc) [ expr $startaddr + [ expr $oc*4 ]]
    regwr_f2s [ expr $hps_mem_base+$addrarray($oc) ] $dataarray($oc)
  }
  puts "Done writting [expr $oc] set of data into hps DDR Memory\n"
  puts "Start reading back for comparison"
  for {set oc 0} {$oc<$count} {incr oc} {
    set data [ regrd_f2s 0x[ format %x [ expr $hps_mem_base+$addrarray($oc) ] ] ]
    if { $data != "0x[format %8.8x $dataarray($oc)]" } {
      puts "Data mismatch!\nwritten:0x[format %8.8x $dataarray($oc)], read:$data, at address:0x[format %8.8x $addrarray($oc)]\n"
      set mm_count [ expr $mm_count+1 ]
    } else {
...
# procedure to write random data to random address of hps DDR memory and read back for comparison
proc mem_rand_test_f2s {{count 1}} {
  global mj_f2s
  global hps_mem_base
  set mm_count 0
  set mt_count 0
