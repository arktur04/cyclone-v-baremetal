# SoCKiT notes: 11/10/2013
#
# device_index forced to 1.
#
# usage: quartus_stp -t ghrd_reset.tcl --cold_reset
#
...
if { [string match "CV SoCKit*" $hardware_name] } {
set cable_name $hardware_name
}
}
...
set device_name [get_device_name $cable_name 1]
set source_instance [get_reset_source_instance $cable_name $device_name]
...
if {$opts(-debug-reset)} {
    source_write_sequence $cable_name $device_name $source_instance {0x0 0x4}
}

if {$opts(-warm-reset)} {
    source_write_sequence $cable_name $device_name $source_instance {0x0 0x2}
}

if {$opts(-cold-reset)} {
    source_write_sequence $cable_name $device_name $source_instance {0x0 0x1}
}
